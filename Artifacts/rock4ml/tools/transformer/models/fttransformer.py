...
from tools.transformer.models.tabtransformer import TransformerBlock
from tensorflow.keras.layers import (
    Dense,
    Flatten,
)
import math as m
from tools.transformer.models.embeddings import CEmbedding, NEmbedding
...
class FTTransformerEncoder(tf.keras.Model):
    def __init__(
        ...
        explainable=False,
        categorical_column_dicts=None,
        layer_name='transformer-dense-'
    ):
        ...
        self.depth = depth
        self.heads = heads
        self.categorical_column_dicts = categorical_column_dicts
        ...
        for layer_id in range(depth):
            self.transformers.append(
                TransformerBlock(
                    ...
                    layer_name=layer_name + str(layer_id),
                )
            )
    ...
    def call(self, _inputs):
        ...
        if isinstance(_inputs, tf.data.Dataset):
            _inputs = tf.concat([tf.cast(batch[0], tf.float32) for batch in _inputs], axis=0)
        if isinstance(_inputs, np.ndarray):
            _inputs = tf.convert_to_tensor(_inputs, dtype=tf.float32)
            
        inputs_dict = {}
        for i, aname in enumerate(self.numerical):
            inputs_dict[aname] = _inputs[:, i:i+1] #[:, tf.newaxis]
        for i, aname in enumerate(self.categorical):
            # inputs_dict[aname] = tf.cast(inputs[:, len(self.numerical) + i][:, tf.newaxis], dtype=tf.string)
            input_int = tf.cast(_inputs[:, len(self.numerical) + i], dtype=tf.int32)
            # extract the mapping
            mapping = self.categorical_column_dicts[i]
            keys = list(mapping.keys())
            values = [mapping[key] for key in keys]
            table = tf.lookup.StaticHashTable(tf.lookup.KeyValueTensorInitializer(keys, values),
                                      default_value="Unknown")
            input_int_replace = table.lookup(input_int)
            
            inputs_dict[aname] = input_int_replace[:, tf.newaxis]
...
class FTTransformer(tf.keras.Model):
    def __init__(
        ...
        self.output_layer = Dense(out_dim, activation=out_activation, name=name + 'last')
