...
from tools.transformer.utils.helper import corrupt_dataset
...
def df_to_dataset_np(
    ...
    df = dataframe.copy()
    if target:
        labels = df.pop(target).values[:, np.newaxis]
        feas = np.float32(df.values)
        print(feas.shape, labels.shape)
        dataset = tf.data.Dataset.from_tensor_slices((feas, labels))
    else:
        feas = df.values
        dataset = tf.data.Dataset.from_tensor_slices(feas)
    if shuffle:
...
def df_to_dataset_np_cat_label(
        dataframe: pd.DataFrame,
        labels: np.array,
        target: str = None,
        shuffle: bool = True,
...
# def df_to_dataset_np(X_np, y_np, shuffle=True, batch_Size=512):
#     import copy
#     X_np_new = copy.copy(X_np)
#     y_np_new = copy.copy(y_np)
#     tensordata = tf.data.Dataset.from_tensor_slices((X_np_new, y_np_new))
#     trans_ds = tensordata.shuffle(buffer_size=len(X_np_new), reshuffle_each_iteration=False, seed=42).batch(batch_Size)
#     return trans_ds
